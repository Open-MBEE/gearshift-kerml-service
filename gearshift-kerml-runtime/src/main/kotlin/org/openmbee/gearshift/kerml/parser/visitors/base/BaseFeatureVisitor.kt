...
abstract class BaseFeatureVisitor<Ctx, Result : Feature> : BaseTypeVisitor<Ctx, Result>() {
    ...
    /**
     * Extract names from a feature identification context without setting them.
     * Returns a pair of (declaredShortName, declaredName), either may be null.
     */
    protected fun extractFeatureIdentificationNames(ctx: KerMLParser.FeatureIdentificationContext?): Pair<String?, String?> {
        if (ctx == null) return null to null
        val shortName = ctx.declaredShortName?.text?.let { unescapeName(it) }
        val name = ctx.declaredName?.text?.let { unescapeName(it) }
        return shortName to name
    }
    ...
    protected fun parseFeatureIdentification(ctx: KerMLParser.FeatureIdentificationContext, feature: Feature) {
        ctx.declaredShortName?.let { feature.declaredShortName = unescapeName(it.text) }
        ctx.declaredName?.let { feature.declaredName = unescapeName(it.text) }
    }
