...
open class IntersectingImpl(
    wrapped: MDMObject,
    engine: MDMEngine
) : RelationshipImpl(wrapped, engine), Intersecting {
    ...
    override var intersectingType: Type
        get() {
            ...
            return linked?.let { Wrappers.wrap(it, engine) as Type }
                ?: throw IllegalStateException("Required association end 'intersectingType' has no linked target")
        }
        set(value) {
            val rawValue = (value as? BaseModelElementImpl)?.wrapped
            engine.setProperty(wrapped.id!!, "intersectingType", rawValue)
        }

    override val typeIntersected: Type?
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "typeIntersected")
