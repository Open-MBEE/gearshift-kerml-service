...
open class ExpressionImpl(
    wrapped: MDMObject,
    engine: MDMEngine
) : StepImpl(wrapped, engine), Expression {

    override val isModelLevelEvaluable: Boolean
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "isModelLevelEvaluable")
            return (rawValue as? Boolean) ?: false
        }

    override val function: KerMLFunction?
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "function")
            return (rawValue as? MDMObject)?.let { Wrappers.wrap(it, engine) as KerMLFunction }
        }

    override val result: Feature
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "result")
            return (rawValue as MDMObject).let { Wrappers.wrap(it, engine) as Feature }
        }

    override fun checkCondition(target: Element): Boolean? {
        val result = engine.invokeOperation(wrapped.id!!, "checkCondition", mapOf("target" to target))
