...
) : TypeImpl(wrapped, engine), Classifier {

    override val ownedSubclassification: Set<Subclassification>
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "ownedSubclassification")
            return (rawValue as? List<*>)
                ?.filterIsInstance<MDMObject>()
                ?.map { Wrappers.wrap(it, engine) as Subclassification }?.toSet()
                ?: emptySet()
        }
}
