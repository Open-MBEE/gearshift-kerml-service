...
/**
 * Implementation of Redefinition.
 * A subsetting where one feature redefines another
 */
open class RedefinitionImpl(
    wrapped: MDMObject,
    engine: GearshiftEngine
) : SubsettingImpl(wrapped, engine), Redefinition {
    ...
    override var redefinedFeature: Feature
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "redefinedFeature")
            val linked = (rawValue as? List<*>)?.filterIsInstance<MDMObject>()?.firstOrNull()
            return linked?.let { Wrappers.wrap(it, engine) as Feature }
                ?: throw IllegalStateException("Required association end 'redefinedFeature' has no linked target")
        }
        set(value) {
            val rawValue = (value as? BaseModelElementImpl)?.wrapped
            engine.setProperty(wrapped.id!!, "redefinedFeature", rawValue)
        }

    override var redefiningFeature: Feature
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "redefiningFeature")
            val linked = (rawValue as? List<*>)?.filterIsInstance<MDMObject>()?.firstOrNull()
            return linked?.let { Wrappers.wrap(it, engine) as Feature }
                ?: throw IllegalStateException("Required association end 'redefiningFeature' has no linked target")
        }
        set(value) {
            val rawValue = (value as? BaseModelElementImpl)?.wrapped
            engine.setProperty(wrapped.id!!, "redefiningFeature", rawValue)
        }
}
