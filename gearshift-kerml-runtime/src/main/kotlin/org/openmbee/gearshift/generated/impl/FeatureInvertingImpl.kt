...
/**
 * Implementation of FeatureInverting.
 * A relationship that specifies a feature as the inverse of another
 */
open class FeatureInvertingImpl(
    wrapped: MDMObject,
    engine: GearshiftEngine
) : RelationshipImpl(wrapped, engine), FeatureInverting {

    override var featureInverted: Feature
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "featureInverted")
            val linked = (rawValue as? List<*>)?.filterIsInstance<MDMObject>()?.firstOrNull()
            return linked?.let { Wrappers.wrap(it, engine) as Feature }
                ?: throw IllegalStateException("Required association end 'featureInverted' has no linked target")
        }
        set(value) {
            val rawValue = (value as? BaseModelElementImpl)?.wrapped
            engine.setProperty(wrapped.id!!, "featureInverted", rawValue)
        }

    override var invertingFeature: Feature
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "invertingFeature")
            val linked = (rawValue as? List<*>)?.filterIsInstance<MDMObject>()?.firstOrNull()
            return linked?.let { Wrappers.wrap(it, engine) as Feature }
                ?: throw IllegalStateException("Required association end 'invertingFeature' has no linked target")
        }
        set(value) {
            val rawValue = (value as? BaseModelElementImpl)?.wrapped
            engine.setProperty(wrapped.id!!, "invertingFeature", rawValue)
        }
