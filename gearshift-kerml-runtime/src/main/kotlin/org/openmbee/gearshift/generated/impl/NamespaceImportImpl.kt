...
/**
 * Implementation of NamespaceImport.
 * An import that brings all visible members of a namespace into another namespace
 */
open class NamespaceImportImpl(
    wrapped: MDMObject,
    engine: MDMEngine
) : ImportImpl(wrapped, engine), NamespaceImport {

    override var importedNamespace: Namespace
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "importedNamespace")
            val linked = (rawValue as? List<*>)?.filterIsInstance<MDMObject>()?.firstOrNull()
            return linked?.let { Wrappers.wrap(it, engine) as Namespace }
                ?: throw IllegalStateException("Required association end 'importedNamespace' has no linked target")
        }
        set(value) {
            val rawValue = (value as? BaseModelElementImpl)?.wrapped
            engine.setProperty(wrapped.id!!, "importedNamespace", rawValue)
        }
