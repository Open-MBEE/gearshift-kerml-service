...
open class AssociationImpl(
    wrapped: MDMObject,
    engine: MDMEngine
) : ClassifierImpl(wrapped, engine), Association {
    ...
    override val associationEnd: Set<Feature>
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "associationEnd")
            return (rawValue as? List<*>)
                ?.filterIsInstance<MDMObject>()
                ?.map { Wrappers.wrap(it, engine) as Feature }?.toSet()
                ?: emptySet()
        }

    override val relatedType: List<Type>
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "relatedType")
            return (rawValue as? List<*>)
                ?.filterIsInstance<MDMObject>()
    ...
    override val sourceType: Type?
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "sourceType")
            return (rawValue as? MDMObject)?.let { Wrappers.wrap(it, engine) as Type }
        }

    override val targetType: Set<Type>
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "targetType")
            return (rawValue as? List<*>)
                ?.filterIsInstance<MDMObject>()
