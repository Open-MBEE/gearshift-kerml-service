...
open class FlowImpl(
    ...
    override val flowEnd: List<FlowEnd>
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "flowEnd")
            return (rawValue as? List<*>)
                ?.filterIsInstance<MDMObject>()
                ?.map { Wrappers.wrap(it, engine) as FlowEnd }
                ?: emptyList()
        }

    override val interaction: List<Interaction>
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "interaction")
            return (rawValue as? List<*>)
                ?.filterIsInstance<MDMObject>()
                ?.map { Wrappers.wrap(it, engine) as Interaction }
                ?: emptyList()
        }

    override val payloadFeature: PayloadFeature?
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "payloadFeature")
            return (rawValue as? MDMObject)?.let { Wrappers.wrap(it, engine) as PayloadFeature }
        }

    override val payloadType: List<Classifier>
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "payloadType")
            return (rawValue as? List<*>)
                ?.filterIsInstance<MDMObject>()
                ?.map { Wrappers.wrap(it, engine) as Classifier }
                ?: emptyList()
        }

    override val sourceOutputFeature: Feature?
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "sourceOutputFeature")
            return (rawValue as? MDMObject)?.let { Wrappers.wrap(it, engine) as Feature }
        }

    override val targetInputFeature: Feature?
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "targetInputFeature")
            return (rawValue as? MDMObject)?.let { Wrappers.wrap(it, engine) as Feature }
        }
