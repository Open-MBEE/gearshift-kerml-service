...
open class DifferencingImpl(
    wrapped: MDMObject,
    engine: MDMEngine
) : RelationshipImpl(wrapped, engine), Differencing {
    ...
    override var differencingType: Type
        get() {
            ...
            return linked?.let { Wrappers.wrap(it, engine) as Type }
                ?: throw IllegalStateException("Required association end 'differencingType' has no linked target")
        }
        set(value) {
            val rawValue = (value as? BaseModelElementImpl)?.wrapped
            engine.setProperty(wrapped.id!!, "differencingType", rawValue)
        }

    override val typeDifferenced: Type?
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "typeDifferenced")
