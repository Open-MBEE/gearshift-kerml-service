...
) : ExpressionImpl(wrapped, engine), BooleanExpression {

    override val predicate: Predicate?
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "predicate")
            return (rawValue as? MDMObject)?.let { Wrappers.wrap(it, engine) as Predicate }
        }
}
