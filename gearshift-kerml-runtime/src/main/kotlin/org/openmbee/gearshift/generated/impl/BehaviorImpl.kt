...
open class BehaviorImpl(
    wrapped: MDMObject,
    engine: MDMEngine
) : ClassImpl(wrapped, engine), Behavior {

    override val parameter: List<Feature>
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "parameter")
    ...
    override val step: Set<Step>
        get() {
            val rawValue = engine.getProperty(wrapped.id!!, "step")
            return (rawValue as? List<*>)
                ?.filterIsInstance<MDMObject>()
                ?.map { Wrappers.wrap(it, engine) as Step }?.toSet()
                ?: emptySet()
        }
}
