...
abstract class BaseTypedVisitor<Ctx, Result : ModelElement> : TypedKerMLVisitor<Ctx, Result> {
    ...
    /**
     * Extract names from an identification context without setting them on an element.
     * Returns a pair of (declaredShortName, declaredName), either of which may be null.
     *
     * This is useful when the name is needed before creating the element (e.g., for
     * generating deterministic elementIds in library context).
     */
    protected fun extractIdentificationNames(identification: KerMLParser.IdentificationContext?): Pair<String?, String?> {
        var shortName: String? = null
        var name: String? = null
        identification?.let { id ->
            id.declaredShortName?.let { shortName = unescapeName(it.text) }
            id.declaredName?.let { name = unescapeName(it.text) }
        }
