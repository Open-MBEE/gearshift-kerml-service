...
class ClassifierVisitor : BaseClassifierVisitor<KerMLParser.ClassifierContext, Classifier>() {
    ...
    override fun visit(ctx: KerMLParser.ClassifierContext, kermlParseContext: KermlParseContext): Classifier {
        ...
        val identification = ctx.classifierDeclaration()?.identification()
        val (shortName, name) = extractIdentificationNames(identification)

        // Create with names for deterministic ID generation in library context
        ...
        // Note: names are already set, parseClassifierDeclaration will overwrite them (no-op)
