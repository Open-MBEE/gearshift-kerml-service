...
/**
 * Implementation of LibraryPackage.
 * A Package that is the container for a model library
 */
open class LibraryPackageImpl(
    className: String,
    metaClass: FrameworkMetaClass,
    engine: MDMEngine
) : PackageImpl(className, metaClass, engine), LibraryPackage {

    /**
     * Create a new LibraryPackage instance.
    ...
    constructor(
    ...
    ) : this("LibraryPackage", engine.schema.getClass("LibraryPackage")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "LibraryPackage")
            if (resolved != null) {
    ...
    override var isStandard: Boolean
        get() {
            val rawValue = getProperty("isStandard")
            return (rawValue as? Boolean) ?: false
        }
        set(value) {
            engine.setProperty(id!!, "isStandard", value)
        }
