...
open class DependencyImpl(
    ...
    engine: MDMEngine
) : RelationshipImpl(className, metaClass, engine), Dependency {
    ...
    constructor(
    ...
    ) : this("Dependency", engine.schema.getClass("Dependency")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "Dependency")
            if (resolved != null) {
    ...
    override var client: List<Element>
        get() {
            val rawValue = engine.getProperty(id!!, "client")
            return (rawValue as? List<*>)?.filterIsInstance<Element>() ?: emptyList()
        }
        set(value) {
            engine.setProperty(id!!, "client", value)
        }

    override var supplier: List<Element>
        get() {
            val rawValue = engine.getProperty(id!!, "supplier")
            return (rawValue as? List<*>)?.filterIsInstance<Element>() ?: emptyList()
        }
        set(value) {
            engine.setProperty(id!!, "supplier", value)
        }
}
