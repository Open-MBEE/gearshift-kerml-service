...
open class RedefinitionImpl(
    ...
    engine: MDMEngine
) : SubsettingImpl(className, metaClass, engine), Redefinition {

    /**
     * Create a new Redefinition instance.
    ...
    constructor(
    ...
    ) : this("Redefinition", engine.schema.getClass("Redefinition")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "Redefinition")
            if (resolved != null) {
    ...
    override var redefinedFeature: Feature
        get() {
            val rawValue = engine.getProperty(id!!, "redefinedFeature")
            return rawValue as Feature
        }
        set(value) {
            engine.setProperty(id!!, "redefinedFeature", value)
        }

    override var redefiningFeature: Feature
        get() {
            val rawValue = engine.getProperty(id!!, "redefiningFeature")
            return rawValue as Feature
        }
        set(value) {
            engine.setProperty(id!!, "redefiningFeature", value)
        }
}
