 ...
 * A Structure that defines how a View is rendered into a presentable format
 */
open class RenderingImpl(
    className: String,
    metaClass: FrameworkMetaClass,
    engine: MDMEngine
) : StructureImpl(className, metaClass, engine), Rendering {
    ...
    constructor(
    ...
    ) : this("Rendering", engine.schema.getClass("Rendering")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "Rendering")
            if (resolved != null) {
