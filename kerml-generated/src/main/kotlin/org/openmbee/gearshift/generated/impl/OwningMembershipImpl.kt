...
open class OwningMembershipImpl(
    ...
    engine: MDMEngine
) : MembershipImpl(className, metaClass, engine), OwningMembership {

    /**
     * Create a new OwningMembership instance.
    ...
    constructor(
    ...
    ) : this("OwningMembership", engine.schema.getClass("OwningMembership")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "OwningMembership")
            if (resolved != null) {
    ...
    override val ownedMemberName: String?
        get() {
            val rawValue = engine.getProperty(id!!, "ownedMemberName")
            return rawValue as? String
        }

    override val ownedMemberShortName: String?
        get() {
            val rawValue = engine.getProperty(id!!, "ownedMemberShortName")
            return rawValue as? String
        }

    override var ownedMemberElement: Element
        get() {
            val rawValue = engine.getProperty(id!!, "ownedMemberElement")
            return rawValue as Element
        }
        set(value) {
            engine.setProperty(id!!, "ownedMemberElement", value)
        }
