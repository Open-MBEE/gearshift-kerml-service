...
open class CrossSubsettingImpl(
    ...
    engine: MDMEngine
) : SubsettingImpl(className, metaClass, engine), CrossSubsetting {

    /**
     * Create a new CrossSubsetting instance.
    ...
    constructor(
    ...
    ) : this("CrossSubsetting", engine.schema.getClass("CrossSubsetting")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "CrossSubsetting")
            if (resolved != null) {
    ...
    override var crossedFeature: Feature
        get() {
            val rawValue = engine.getProperty(id!!, "crossedFeature")
            return rawValue as Feature
        }
        set(value) {
            engine.setProperty(id!!, "crossedFeature", value)
        }

    override val crossingFeature: Feature
        get() {
            val rawValue = engine.getProperty(id!!, "crossingFeature")
            return rawValue as Feature
        }
}
