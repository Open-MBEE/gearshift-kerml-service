...
open class ExpressionImpl(
    ...
    engine: MDMEngine
) : StepImpl(className, metaClass, engine), Expression {
    ...
    constructor(
    ...
    ) : this("Expression", engine.schema.getClass("Expression")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "Expression")
            if (resolved != null) {
    ...
    override val isModelLevelEvaluable: Boolean
        get() {
            val rawValue = engine.getProperty(id!!, "isModelLevelEvaluable")
            return (rawValue as? Boolean) ?: false
        }

    override val function: KerMLFunction?
        get() {
            val rawValue = engine.getProperty(id!!, "function")
            return rawValue as? KerMLFunction
        }

    override val result: Feature
        get() {
            val rawValue = engine.getProperty(id!!, "result")
            return rawValue as Feature
        }

    override fun checkCondition(target: Element): Boolean? {
        val result = engine.invokeOperation(id!!, "checkCondition", mapOf("target" to target))
        return result as? Boolean
    }

    override fun evaluate(target: Element): List<Element> {
        val result = engine.invokeOperation(id!!, "evaluate", mapOf("target" to target))
        return (result as? List<*>)?.filterIsInstance<Element>() ?: emptyList()
    }

    override fun modelLevelEvaluable(visited: List<Feature>): Boolean? {
        val result = engine.invokeOperation(id!!, "modelLevelEvaluable", mapOf("visited" to visited))
        return result as? Boolean
    }
}
