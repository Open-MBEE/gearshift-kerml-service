...
open class SubViewImpl(
    className: String,
    metaClass: FrameworkMetaClass,
    engine: MDMEngine
) : FeatureImpl(className, metaClass, engine), SubView {

    /**
     * Create a new SubView instance.
    ...
    constructor(
    ...
    ) : this("SubView", engine.schema.getClass("SubView")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "SubView")
            if (resolved != null) {
    ...
    override val view: View?
        get() {
            val rawValue = engine.getProperty(id!!, "view")
            return rawValue as? View
        }
