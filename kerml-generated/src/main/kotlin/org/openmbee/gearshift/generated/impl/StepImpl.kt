...
open class StepImpl(
    ...
    engine: MDMEngine
) : FeatureImpl(className, metaClass, engine), Step {
    ...
    constructor(
    ...
    ) : this("Step", engine.schema.getClass("Step")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "Step")
            if (resolved != null) {
    ...
    override val behavior: List<Behavior>
        get() {
            val rawValue = engine.getProperty(id!!, "behavior")
            return (rawValue as? List<*>)?.filterIsInstance<Behavior>() ?: emptyList()
        }
