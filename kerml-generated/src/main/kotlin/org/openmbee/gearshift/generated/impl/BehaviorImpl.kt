 ...
 */
open class BehaviorImpl(
    className: String,
    metaClass: FrameworkMetaClass,
    engine: MDMEngine
) : ClassImpl(className, metaClass, engine), Behavior {
    ...
    constructor(
    ...
    ) : this("Behavior", engine.schema.getClass("Behavior")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "Behavior")
            if (resolved != null) {
    ...
    override val parameter: List<Feature>
        get() {
            val rawValue = engine.getProperty(id!!, "parameter")
            return (rawValue as? List<*>)?.filterIsInstance<Feature>() ?: emptyList()
        }

    override val step: Set<Step>
        get() {
            val rawValue = engine.getProperty(id!!, "step")
            return (rawValue as? List<*>)?.filterIsInstance<Step>()?.toSet() ?: emptySet()
        }
}
