...
/**
 * Implementation of Intersecting.
 * A relationship that specifies the intersecting type for a type
 */
open class IntersectingImpl(
    className: String,
    metaClass: FrameworkMetaClass,
    engine: MDMEngine
) : RelationshipImpl(className, metaClass, engine), Intersecting {

    /**
     * Create a new Intersecting instance.
    ...
    constructor(
    ...
    ) : this("Intersecting", engine.schema.getClass("Intersecting")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "Intersecting")
            if (resolved != null) {
    ...
    override var intersectingType: Type
        get() {
            val rawValue = engine.getProperty(id!!, "intersectingType")
            return rawValue as Type
        }
        set(value) {
            engine.setProperty(id!!, "intersectingType", value)
        }

    override val typeIntersected: Type?
        get() {
            val rawValue = engine.getProperty(id!!, "typeIntersected")
            return rawValue as? Type
        }
}
