...
/**
 * Implementation of LiteralRational.
 * A LiteralRational is a LiteralExpression that provides a Rational value as a result.
 */
open class LiteralRationalImpl(
    className: String,
    metaClass: FrameworkMetaClass,
    engine: MDMEngine
) : LiteralExpressionImpl(className, metaClass, engine), LiteralRational {

    /**
     * Create a new LiteralRational instance.
    ...
    constructor(
    ...
    ) : this("LiteralRational", engine.schema.getClass("LiteralRational")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "LiteralRational")
            if (resolved != null) {
    ...
    override var value: Double
        get() {
            val rawValue = getProperty("value")
            return (rawValue as? Double) ?: 0.0
        }
        set(value) {
            engine.setProperty(id!!, "value", value)
        }
}
