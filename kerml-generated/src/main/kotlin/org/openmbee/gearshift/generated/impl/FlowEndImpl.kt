...
/**
 * Implementation of FlowEnd.
 * A FlowEnd is a Feature that is one of the connectorEnds giving the source or target of a Flow.
 */
open class FlowEndImpl(
    className: String,
    metaClass: FrameworkMetaClass,
    engine: MDMEngine
) : FeatureImpl(className, metaClass, engine), FlowEnd {

    /**
     * Create a new FlowEnd instance.
    ...
    constructor(
    ...
    ) : this("FlowEnd", engine.schema.getClass("FlowEnd")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "FlowEnd")
            if (resolved != null) {
