 ...
 * A relationship that specifies the differencing type for a type
 */
open class DifferencingImpl(
    className: String,
    metaClass: FrameworkMetaClass,
    engine: MDMEngine
) : RelationshipImpl(className, metaClass, engine), Differencing {

    /**
     * Create a new Differencing instance.
    ...
    constructor(
    ...
    ) : this("Differencing", engine.schema.getClass("Differencing")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "Differencing")
            if (resolved != null) {
    ...
    override var differencingType: Type
        get() {
            val rawValue = engine.getProperty(id!!, "differencingType")
            return rawValue as Type
        }
        set(value) {
            engine.setProperty(id!!, "differencingType", value)
        }

    override val typeDifferenced: Type?
        get() {
            val rawValue = engine.getProperty(id!!, "typeDifferenced")
            return rawValue as? Type
        }
}
