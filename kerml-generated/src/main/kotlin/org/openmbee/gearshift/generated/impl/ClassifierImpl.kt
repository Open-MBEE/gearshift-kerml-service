 ...
 */
open class ClassifierImpl(
    className: String,
    metaClass: FrameworkMetaClass,
    engine: MDMEngine
) : TypeImpl(className, metaClass, engine), Classifier {
    ...
    constructor(
    ...
    ) : this("Classifier", engine.schema.getClass("Classifier")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "Classifier")
            if (resolved != null) {
    ...
    override val ownedSubclassification: Set<Subclassification>
        get() {
            val rawValue = engine.getProperty(id!!, "ownedSubclassification")
            return (rawValue as? List<*>)?.filterIsInstance<Subclassification>()?.toSet() ?: emptySet()
        }
}
