...
open class AnnotatingElementImpl(
    ...
    engine: MDMEngine
) : ElementImpl(className, metaClass, engine), AnnotatingElement {

    /**
     * Create a new AnnotatingElement instance.
    ...
    constructor(
    ...
    ) : this("AnnotatingElement", engine.schema.getClass("AnnotatingElement")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "AnnotatingElement")
            if (resolved != null) {
    ...
    override val annotation: List<KerMLAnnotation>
        get() {
            val rawValue = engine.getProperty(id!!, "annotation")
    ...
    override val ownedAnnotatingRelationship: List<KerMLAnnotation>
        get() {
            val rawValue = engine.getProperty(id!!, "ownedAnnotatingRelationship")
            return (rawValue as? List<*>)?.filterIsInstance<KerMLAnnotation>() ?: emptyList()
        }

    override var owningAnnotatingRelationship: KerMLAnnotation?
        get() {
            val rawValue = engine.getProperty(id!!, "owningAnnotatingRelationship")
            return rawValue as? KerMLAnnotation
        }
        set(value) {
            engine.setProperty(id!!, "owningAnnotatingRelationship", value)
        }
}
