...
open class FunctionImpl(
    ...
    engine: MDMEngine
) : BehaviorImpl(className, metaClass, engine), KerMLFunction {
    ...
    constructor(
    ...
    ) : this("Function", engine.schema.getClass("Function")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "Function")
            if (resolved != null) {
    ...
    override val expression: Set<Expression>
        get() {
            val rawValue = engine.getProperty(id!!, "expression")
            return (rawValue as? List<*>)?.filterIsInstance<Expression>()?.toSet() ?: emptySet()
        }
