...
open class AnnotationImpl(
    ...
    engine: MDMEngine
) : RelationshipImpl(className, metaClass, engine), KerMLAnnotation {
    ...
    constructor(
    ...
    ) : this("Annotation", engine.schema.getClass("Annotation")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "Annotation")
            if (resolved != null) {
    ...
    override var annotatedElement: Element
        get() {
            val rawValue = engine.getProperty(id!!, "annotatedElement")
            return rawValue as Element
        }
        set(value) {
            engine.setProperty(id!!, "annotatedElement", value)
        }

    override val annotatingElement: AnnotatingElement
        get() {
            val rawValue = engine.getProperty(id!!, "annotatingElement")
            return rawValue as AnnotatingElement
        }

    override var ownedAnnotatingElement: AnnotatingElement?
        get() {
            val rawValue = engine.getProperty(id!!, "ownedAnnotatingElement")
            return rawValue as? AnnotatingElement
        }
        set(value) {
            engine.setProperty(id!!, "ownedAnnotatingElement", value)
        }

    override val owningAnnotatedElement: Element?
        get() {
            val rawValue = engine.getProperty(id!!, "owningAnnotatedElement")
            return rawValue as? Element
        }

    override val owningAnnotatingElement: AnnotatingElement?
        get() {
            val rawValue = engine.getProperty(id!!, "owningAnnotatingElement")
            return rawValue as? AnnotatingElement
        }
}
