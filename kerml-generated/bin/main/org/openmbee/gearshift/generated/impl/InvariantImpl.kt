 ...
 */
open class InvariantImpl(
    className: String,
    metaClass: FrameworkMetaClass,
    engine: MDMEngine
) : BooleanExpressionImpl(className, metaClass, engine), Invariant {
    ...
    constructor(
    ...
    ) : this("Invariant", engine.schema.getClass("Invariant")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "Invariant")
            if (resolved != null) {
    ...
    override var isNegated: Boolean
        get() {
            val rawValue = getProperty("isNegated")
            return (rawValue as? Boolean) ?: false
        }
        set(value) {
            engine.setProperty(id!!, "isNegated", value)
        }
}
