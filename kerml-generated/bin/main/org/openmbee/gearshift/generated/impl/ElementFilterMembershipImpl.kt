...
/**
 * Implementation of ElementFilterMembership.
 * A Membership between a Namespace and a model-level evaluable Boolean-valued Expression, asserting that imported members of the Namespace should be filtered using the condition Expression
 */
open class ElementFilterMembershipImpl(
    className: String,
    metaClass: FrameworkMetaClass,
    engine: MDMEngine
) : OwningMembershipImpl(className, metaClass, engine), ElementFilterMembership {

    /**
     * Create a new ElementFilterMembership instance.
    ...
    constructor(
    ...
    ) : this("ElementFilterMembership", engine.schema.getClass("ElementFilterMembership")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "ElementFilterMembership")
            if (resolved != null) {
    ...
    override var condition: Expression
        get() {
            val rawValue = engine.getProperty(id!!, "condition")
            return rawValue as Expression
        }
        set(value) {
            engine.setProperty(id!!, "condition", value)
        }
}
