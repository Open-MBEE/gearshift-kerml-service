 ...
 */
open class BooleanExpressionImpl(
    className: String,
    metaClass: FrameworkMetaClass,
    engine: MDMEngine
) : ExpressionImpl(className, metaClass, engine), BooleanExpression {
    ...
    constructor(
    ...
    ) : this("BooleanExpression", engine.schema.getClass("BooleanExpression")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "BooleanExpression")
            if (resolved != null) {
    ...
    override val predicate: Predicate?
        get() {
            val rawValue = engine.getProperty(id!!, "predicate")
            return rawValue as? Predicate
        }
}
