...
open class ReferenceSubsettingImpl(
    ...
    engine: MDMEngine
) : SubsettingImpl(className, metaClass, engine), ReferenceSubsetting {

    /**
     * Create a new ReferenceSubsetting instance.
    ...
    constructor(
    ...
    ) : this("ReferenceSubsetting", engine.schema.getClass("ReferenceSubsetting")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "ReferenceSubsetting")
            if (resolved != null) {
    ...
    override var referencedFeature: Feature
        get() {
            val rawValue = engine.getProperty(id!!, "referencedFeature")
            return rawValue as Feature
        }
        set(value) {
            engine.setProperty(id!!, "referencedFeature", value)
        }

    override val referencingFeature: Feature
        get() {
            val rawValue = engine.getProperty(id!!, "referencingFeature")
            return rawValue as Feature
        }
}
