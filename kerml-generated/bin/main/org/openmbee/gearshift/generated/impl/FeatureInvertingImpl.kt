...
open class FeatureInvertingImpl(
    ...
    engine: MDMEngine
) : RelationshipImpl(className, metaClass, engine), FeatureInverting {

    /**
     * Create a new FeatureInverting instance.
    ...
    constructor(
    ...
    ) : this("FeatureInverting", engine.schema.getClass("FeatureInverting")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "FeatureInverting")
            if (resolved != null) {
    ...
    override var featureInverted: Feature
        get() {
            val rawValue = engine.getProperty(id!!, "featureInverted")
            return rawValue as Feature
        }
        set(value) {
            engine.setProperty(id!!, "featureInverted", value)
        }

    override var invertingFeature: Feature
        get() {
            val rawValue = engine.getProperty(id!!, "invertingFeature")
            return rawValue as Feature
        }
        set(value) {
            engine.setProperty(id!!, "invertingFeature", value)
        }
