...
open class MultiplicityRangeImpl(
    ...
    engine: MDMEngine
) : MultiplicityImpl(className, metaClass, engine), MultiplicityRange {

    /**
     * Create a new MultiplicityRange instance.
    ...
    constructor(
    ...
    ) : this("MultiplicityRange", engine.schema.getClass("MultiplicityRange")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "MultiplicityRange")
            if (resolved != null) {
    ...
    override val bound: List<Expression>
        get() {
            val rawValue = engine.getProperty(id!!, "bound")
            return (rawValue as? List<*>)?.filterIsInstance<Expression>() ?: emptyList()
        }

    override val lowerBound: Expression?
        get() {
            val rawValue = engine.getProperty(id!!, "lowerBound")
            return rawValue as? Expression
        }

    override val upperBound: Expression
        get() {
            val rawValue = engine.getProperty(id!!, "upperBound")
            return rawValue as Expression
        }

    override fun hasBounds(lower: Int, upper: Int): Boolean? {
        val result = engine.invokeOperation(id!!, "hasBounds", mapOf("lower" to lower, "upper" to upper))
        return result as? Boolean
    }

    override fun valueOf(bound: Expression?): Int? {
        val result = engine.invokeOperation(id!!, "valueOf", mapOf("bound" to bound))
