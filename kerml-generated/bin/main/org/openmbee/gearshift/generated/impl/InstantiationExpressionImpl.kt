...
abstract class InstantiationExpressionImpl(
    ...
    engine: MDMEngine
) : ExpressionImpl(className, metaClass, engine), InstantiationExpression {


    override val argument: List<Expression>
        get() {
            val rawValue = engine.getProperty(id!!, "argument")
            return (rawValue as? List<*>)?.filterIsInstance<Expression>() ?: emptyList()
        }

    override val instantiatedType: Type
        get() {
            val rawValue = engine.getProperty(id!!, "instantiatedType")
