 ...
 */
open class CommentImpl(
    className: String,
    metaClass: FrameworkMetaClass,
    engine: MDMEngine
) : AnnotatingElementImpl(className, metaClass, engine), Comment {
    ...
    constructor(
    ...
    ) : this("Comment", engine.schema.getClass("Comment")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "Comment")
            if (resolved != null) {
    ...
    override var locale: String?
        get() {
            val rawValue = getProperty("locale")
            return rawValue as? String
        }
        set(value) {
            engine.setProperty(id!!, "locale", value)
        }
}
