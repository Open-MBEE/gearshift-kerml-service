...
open class SubclassificationImpl(
    ...
    engine: MDMEngine
) : SpecializationImpl(className, metaClass, engine), Subclassification {

    /**
     * Create a new Subclassification instance.
    ...
    constructor(
    ...
    ) : this("Subclassification", engine.schema.getClass("Subclassification")!!, engine) {
        ...
        parent?.let { owner ->
            ...
            val resolved = resolver.resolve(owner.className, "Subclassification")
            if (resolved != null) {
    ...
    override val owningClassifier: Classifier?
        get() {
            val rawValue = engine.getProperty(id!!, "owningClassifier")
            return rawValue as? Classifier
        }

    override var subclassifier: Classifier
        get() {
            val rawValue = engine.getProperty(id!!, "subclassifier")
            return rawValue as Classifier
        }
        set(value) {
            engine.setProperty(id!!, "subclassifier", value)
        }

    override var superclassifier: Classifier
        get() {
            val rawValue = engine.getProperty(id!!, "superclassifier")
            return rawValue as Classifier
        }
        set(value) {
            engine.setProperty(id!!, "superclassifier", value)
        }
}
