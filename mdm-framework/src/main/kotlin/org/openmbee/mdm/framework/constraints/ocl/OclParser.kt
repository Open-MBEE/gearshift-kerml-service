...
import org.openmbee.mdm.framework.constraints.ocl.antlr.OCLBaseVisitor
import org.openmbee.mdm.framework.constraints.ocl.antlr.OCLLexer
import org.openmbee.mdm.framework.constraints.ocl.antlr.OCLParser
...
object OclParser {
    ...
    private class AstBuilder : OCLBaseVisitor<OclExpression>() {
        ...
        override fun visitPostfixExpression(ctx: OCLParser.PostfixExpressionContext): OclExpression {
            ...
            // Apply suffixes in their original order (left-to-right associativity).
            // The children include the primaryExpression followed by interleaved dotSuffix and arrowSuffix.
            // We need to process them in sequence order, not all dots then all arrows.
            for (i in 1 until ctx.childCount) {
                val child = ctx.getChild(i)
                when (child) {
                    is OCLParser.DotSuffixContext -> expr = applyDotSuffix(expr, child)
                    is OCLParser.ArrowSuffixContext -> expr = applyArrowSuffix(expr, child)
                }
            }
        ...
        private fun applyDotSuffix(source: OclExpression, ctx: OCLParser.DotSuffixContext): OclExpression {
            // Type operations with dot notation (e.g., s.oclIsKindOf(Type))
            if (ctx.typeDotSuffix() != null) {
                return applyTypeDotSuffix(source, ctx.typeDotSuffix())
            }
        ...
        override fun visitStandaloneOperationCall(ctx: OCLParser.StandaloneOperationCallContext): OclExpression {
            // Standalone operation calls have implicit iterator variable as source
            // This allows operations like typingFeatures() to work in iterator contexts like closure()
            val source = VariableExp("_it")
