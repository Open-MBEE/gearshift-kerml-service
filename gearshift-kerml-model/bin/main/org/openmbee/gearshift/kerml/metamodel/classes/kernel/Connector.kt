...
fun createConnectorMetaClass() = MetaClass(
    ...
    constraints = listOf(
        ...
        MetaConstraint(
            name = "checkConnectorEndRedefinition",
            type = ConstraintType.VERIFICATION,
            expression = """
                ...
                forAll(supertype |
                    ...
                    connectorEnd->at(i).redefines(supertype.endFeature->at(i)))
            """.trimIndent(),
            description = "If a Connector has an owningType, then each of its connectorEnds must redefine the endFeature at the corresponding position in each supertype of the owningType, if any."
        ),
        MetaConstraint(
    ...
    semanticBindings = listOf(
        // TypeFeaturing: Connector is featured by defaultFeaturingType when it has no explicit featuringType
        SemanticBinding(
            name = "connectorDefaultTypeFeaturingBinding",
            baseConcept = "self::defaultFeaturingType",
            bindingKind = BindingKind.TYPE_FEATURES,
            condition = BindingCondition.CollectionEmpty("featuringType")
        ),
