...
enum class BindingKind {
    ...
    /**
     * Creates an implied Redefinition relationship.
     * Used for Features that implicitly redefine a base library feature.
     */
    REDEFINES,

    /**
     * Creates an implied TypeFeaturing relationship.
     * Used for Features that are implicitly featured by a type.
     */
    TYPE_FEATURES
}
...
sealed class BindingCondition {
    ...
    /**
     * Binding applies when a collection property is empty.
     * Used for conditions like `featuringType->isEmpty()`.
     *
     * @property property The collection property name to check
     */
    data class CollectionEmpty(val property: String) : BindingCondition()
    ...
    /**
     * Binding applies when the element has an owningFeatureMembership.
     * Used for TypeFeaturing where the featuringType is the owningType.
     */
    data object HasOwningFeatureMembership : BindingCondition()

    /**
     * Binding applies when the element is an end feature with an owningType.
     * Used for end feature Redefinition where the redefined feature is computed
     * from the corresponding position in supertypes.
     */
    data object IsEndWithOwningType : BindingCondition()
}
